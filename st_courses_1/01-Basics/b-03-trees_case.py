...
with col_2:
    ...
    st.subheader("distplot about age")
    fig_1, ax_1 = plt.subplots()
    ax_1 = sns.distplot(df['age_days'], color=graph_color)
    plt.xlabel('Age (Days)')
    plt.title("Tree Age")
    st.pyplot(fig_1)
...
with col_3:
    ...
    st.subheader("distplot about Tree Width")
    fig_2, ax_2 = plt.subplots()
    # 将 NaN 值填充为 0，然后转换为整数类型
    df['dbh'] = df['dbh'].fillna(0).astype(int)
    ax_2 = sns.distplot(df['dbh'], color=graph_color)
